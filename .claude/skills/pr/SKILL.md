...
1. `gh pr view --json number,title,url` 로 현재 브랜치에 열린 PR이 있는지 확인
2. `git diff develop...HEAD`와 `git log develop..HEAD --oneline`으로 변경사항 분석
3. 변경된 파일들을 읽고 어떤 모듈이 영향받는지 파악
4. PR 제목 작성: 컨벤션 prefix 사용 (`feat:`, `fix:`, `refactor:`, `docs:`, `chore:`, `test:`) + 한글 설명
5. `.github/pull_request_template.md` 템플릿 기반으로 PR 본문 작성:
   - Summary: 변경사항 요약

...
class DeleteFileUseCaseTest {
    ...
    fun setUp() {
        fileAdaptor = mockk()
        s3Service = mockk()
        useCase = DeleteFileUseCase(fileAdaptor, s3Service)
    }
    ...
    fun `파일을 정상 삭제한다`() {
        ...
        verify { fileAdaptor.findById(fileId) }
        verify { fileAdaptor.delete(fileId) }
        verify { s3Service.deleteObject(file.storedFilename) }
    }

    @Test
    fun `DB 조회 후 삭제, S3 삭제 순서로 실행된다`() {
        val file = createFile()
        every { fileAdaptor.findById(fileId) } returns file
        every { fileAdaptor.delete(fileId) } just runs
        every { s3Service.deleteObject(file.storedFilename) } just runs

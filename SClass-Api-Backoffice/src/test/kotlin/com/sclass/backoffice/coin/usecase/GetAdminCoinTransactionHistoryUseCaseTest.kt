...
class GetAdminCoinTransactionHistoryUseCaseTest {
    ...
    fun `userId 기준으로 트랜잭션 히스토리를 반환한다`() {
        ...
        val items =
            listOf(
                CoinTransactionGroupDto(CoinTransactionType.ISSUED, 500L, "ref-001", "멤버십 코인 지급", now.minusDays(2)),
                CoinTransactionGroupDto(CoinTransactionType.DEDUCTED, 100L, "ref-002", "의뢰 생성 코인 차감", now.minusDays(1)),
            )
        every {
